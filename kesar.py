...
def page(uid, *contents, **kwargs):
    if uid is None:
        return "<!DOCTYPE html>" + html_()(
            ...
            form_(action="", method="POST", autocomplete="off")(*contents),
        )
    ...
    return "<!DOCTYPE html>" + html_()(
        ...
        style_()(static_("static/styles.css")),
        script_()(
            """
...
def static_(path):
    ...
    if path not in static_cache or static_cache[path]["mtime"] != mtime:
        with open(path) as f:
            static_cache[path] = {"mtime": mtime, "value": f.read()}
...
def exit_survey_(*qs):
    return div_()(
        ...
        p_()(
            "Answer the following questions and press 'next' to finish the study!"
        ),
        *[
            section_()(p_()(q), textarea_(name=f"Q{i + 1}", required=True)(""))
...
def kesar(
        ...
        def do_POST(self):
            ...
            try:
                ...
                if type(e.value) == tuple and len(e.value) == 2:
                    ...
                    data, bye = (
                        e.value,
                        "You have completed the experiment and can safely close this window.",
                    )
    ...
    def refresh_daemon():
        ...
        print(
            f"** I am watching {init_file} for changes, and will reboot if it is changed."
        )
        ...
        while True:
            ...
            if current_mtime > init_mtime:
                ...
                print(
                    f"""** I am refreshing the server by running $ python3 {' '.join(sys.argv)}."""
                )
                print()
                with logfile_lock:
    ...
    ip = get_ip()
    print(
        f"** I am launching the server at http://{ip}:{port} - press CTRL-C to stop me anytime."
    )
    ...
    if remote_host is not None and remote_port is not None:
        ...
        print(
            f"** I am launching the remote server at http://{remote_host}:{remote_port}."
        )
        print(f"   $ {ssh_cmd}")
